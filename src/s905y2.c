 ...
 * offset input arguments for s905y2_port_regaddr().
 */
typedef enum {
...
volatile uint32_t *s905y2_gpio = (uint32_t *)MAP_FAILED;
 ...
 */
static uint32_t
s905y2_port_regaddr(uint32_t pin, s905y2_port_offset_t regoff)
{
	uint32_t addr;
	 ...
	 * selected pin.
	 */
	addr = (uint32_t)s905y2_gpio + SUNXI_GPIO_OFFSET
 ...
 */
static uint32_t
s905y2_peri_read(volatile uint32_t *paddr)
{
	uint32_t val;
...
static void
s905y2_peri_write(volatile uint32_t *paddr, uint32_t val)
{
	__sync_synchronize();
	*paddr = val;
	__sync_synchronize();
...
#ifdef NOTYET
static uint32_t
s905y2_peri_read_nb(volatile uint32_t *paddr)
{
	uint32_t val;
...
static void
s905y2_peri_write_nb(volatile uint32_t *paddr, uint32_t val)
{
	*paddr = val;
}
#endif
 ...
 */
uint8_t
s905y2_gpio_lev(uint32_t pin)
{
	volatile uint32_t *paddr;
	uint32_t value;

	paddr = (uint32_t *)s905y2_port_regaddr(pin, SUNXI_PORT_OFFSET_DAT);
	value = s905y2_peri_read(paddr);

	return (value & (1 << (pin % 32))) ? 1 : 0;
}

void
s905y2_gpio_set(uint32_t pin)
{
	volatile uint32_t *paddr;
	uint32_t value;
	...
	s905y2_peri_write(paddr, (value | (1 << (pin % 32))));
}

void
s905y2_gpio_clr(uint32_t pin)
{
	volatile uint32_t *paddr;
	uint32_t value;
 ...
 */
void
s905y2_gpio_set_pud(uint32_t pin, uint8_t status)
{
	volatile uint32_t *paddr;
	uint32_t value;

	paddr = (uint32_t *)s905y2_port_regaddr(pin, SUNXI_PORT_OFFSET_PUL);
	value = s905y2_peri_read(paddr);
 ...
 */
void
s905y2_gpio_fsel(uint32_t pin, uint8_t mode)
{
	volatile uint32_t *paddr;
	uint32_t value;
	 ...
	 */
	paddr = (uint32_t *)s905y2_port_regaddr(pin, SUNXI_PORT_OFFSET_CFG);
	value = s905y2_peri_read(paddr);
	value &= ~(SUNXI_FSEL_MASK << SUNXI_PIN_SELECT(pin));

	switch (mode) {
	case SUNXI_FSEL_INPUT:
		s905y2_peri_write(paddr, value);
		break;
 ...
 */
void
s905y2_gpio_write(uint32_t pin, uint32_t on)
{
	if (on)
		s905y2_gpio_set(pin);
	else
...
int
s905y2_init(int unused)
{
	int fd;

	if ((fd = open("/dev/mem", O_RDWR | O_SYNC | O_CLOEXEC)) < 0) {
		fprintf(stderr, "s905y2_init: open /dev/mem failed: %s\n",
		    strerror(errno));
		return 0;
	}

	s905y2_gpio = (uint32_t *)mmap(0, SUNXI_GPIO_SIZE,
	    PROT_READ | PROT_WRITE, MAP_SHARED, fd, SUNXI_GPIO_BASE);

	if (s905y2_gpio == MAP_FAILED) {
		fprintf(stderr, "s905y2_init: mmap failed: %s\n",
		    strerror(errno));
		return 0;
	}
